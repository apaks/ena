...
@Modification by Zhaolun Will Su

Added the function for timestamps interpolation and maximized the performance.
The calling convention is exactly the same as previous versions.
...
MAX_NUMBER_OF_CONTINUOUS_SAMPLES = 1e7 # 1e8
...
#    f.close
    ...
    print 'interpolating timestamps using bufferSize', header['bufferSize'], '\n'
    new_stamp = [];
    ss = recordNumber;
    i = 0;
    # while i < (ss):
    #     lower = float(timestamps[i])/30000;
    #     ##upper = float(timestamps[i+1])/30000;
    #     upper = lower +  header['bufferSize']/30000
    #     length = 1024;
    #     #temp = [lower + x*(upper-lower)/length for x in range(length)];
    #     temp = np.linspace(lower, upper, num = header['bufferSize'])
    #     temp = temp.tolist();
    #     new_stamp = new_stamp + temp;
    #     i = i + 1;
    #     #print i
    #     #print ss
    #     print i, " out of ", ss," datapoints complete         \r",

    #np_array = np.linspace(timestamps[0]/30000, (timestamps[0]+SAMPLES_PER_RECORD*ss-1)/30000, num=SAMPLES_PER_RECORD*ss)

    np_array = np.linspace(0, 0, num=0)

    #In order to maximize performance the frequency of doing numpy list concatenation should be minimized
    #This is done by grouping as many data points for interpolation as possible
    #Between each time stamps, SAMPLES_PER_RECORD = 1024 amount of data points are omitted.
    #Sometimes there would be gaps between time stamps. The exact manner for interpolating these gaps is provided below

    interval_count = None;

    while i < ss:
        base_count = 0;
        lower_base = timestamps[i];
        #stop counting data points when encounters a gap or reaches the end of the list
        while timestamps[i+1] - timestamps[i] == SAMPLES_PER_RECORD and i < ss:  
            i = i + 1;
            base_count = base_count + 1;
        higher_base = timestamps[i-1] + 1023;
        temp = np.linspace(lower_base, higher_base, num=SAMPLES_PER_RECORD*base_count)
        np_array = np.concatenate((np_array, temp), axis=0)
        
        ##Interpolating the Gap
        ##if it is the end of the list we don't have to do this appending anymore
        if i < ss:
            temp = np.linspace(timestamps[i], timestamps[i] + 1023, num=SAMPLES_PER_RECORD)
            np_array = np.concatenate((np_array, temp), axis=0)
            i = i + 1
    ...
    #while i < ss:
    #    lower = timestamps[i];
    #    length = SAMPLES_PER_RECORD;
    #    temp = np.linspace(lower, lower+1023, 1024)
    #    np_array = np.concatenate((np_array, temp), axis=0)
    #    i = i + 1;
    #    print i, " out of ", ss," datapoints complete         \r",
    ...
    #temp = [float(timestamps[ss-2]) + x*(float(timestamps[ss-1])-float(timestamps[ss-2]))/1024 for x in range(1024)];
    ##temp = np.linspace(timestamps[ss-2]/30000, timestamps[ss-1]/30000, num = header['bufferSize']);
    ##temp = temp.tolist();
    ##new_stamp = new_stamp + temp;
    #
    #    

...
now = datetime.datetime.now()#NEW
date = now.strftime("%Y-%m-%d_%H-%M-%S")#NEW

foldername =  '001_videos_' + str(date) #NEW
...
#print 'opening cap:', datetime.datetime.now().time()
if capture.isOpened():
    capture.set(cv2.cv.CV_CAP_PROP_FRAME_WIDTH, 160)#NEW
    capture.set(cv2.cv.CV_CAP_PROP_FRAME_HEIGHT, 120)#NEW #Check obj analysis crop for changing x,y
    capture.set(cv2.cv.CV_CAP_PROP_FPS, 40)#NEW
    ###^Must match writer fps. Can increase to ~30 if needed
    ###CV_CAP_PROP_BRIGHTNESS Brightness of the image (only for cameras).
    #capture.set(cv2.cv.CV_CAP_PROP_FRAME_WIDTH, 1024)
    #capture.set(cv2.cv.CV_CAP_PROP_FRAME_HEIGHT, 640)
...
while True:
    ...
    if len(buf) > 0: #If any data is received
        ...
        fname = os.path.join(foldername, fname)#NEW
        
        writer = cv2.VideoWriter(filename=fname, fourcc=cv.CV_FOURCC(*'iYUV'), ###Why use this encoding for video, exactly???
        #fourcc=cv.CV_FOURCC(*'X264'),
        fps=40,#NEW ######This is not always staying consistent w/rec time.
        frameSize=(160, 120))#NEW # MUST MATCH CAPTURE
        ...
        clientsocket.send('Camera start') #Trigger stimulus on this message?
        #Keep this message just before frame numbers to improve synchrony

        for i in range(160): #total frames captured, captured at rate listed above (supposedly) (40 frames per second? 400 = 10.0s, 240 = 6.0s, 1/40 * 160 = 4.0s.May not always be perfectly coordinated w recording time, but should be most of the time

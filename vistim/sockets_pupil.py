...
foldername =  '001_videos_' + str(date) 
...
if capture.isOpened():
	...
	capture.set(cv2.CAP_PROP_FRAME_HEIGHT, 120) #Check obj analysis crop for changing x,y
	# capture.set(cv2.CAP_PROP_FPS, 40)
	# time.sleep(2)

	# capture.set(15, 0.099)
...
while True:
    ...
    if len(buf) > 0: #If any data is received
        ...
        #fourcc=cv.CV_FOURCC(*'X264'),
                                fps=20, ######This is not always staying consistent w/rec time.
        ...
        for i in range(40): #total frames captured, captured at rate listed above (supposedly) (20 frames per second? 1/20 * 80 = 4.0s.  May not always be perfectly coordinated w recording time, but should be most of the time
            ...
            cv2.imshow("Preview", np.hstack([gray, equ]))
            cv2.waitKey(1)
        print 'Camera start at time:', start_time

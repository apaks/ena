...
foldername =  r'C:\Users\Achub_Lab\Desktop\videos\001_videos_' + str(date) 
...
while True:
    ...
    if len(buf) > 0: #If any data is received
        ...
        if buf == 'Finished': #IFF 'Finished' is received, stop video cap
            ...
            capture.release()
            break;
        cc =cv2.VideoWriter_fourcc(*'LAGS')
        ...
        for i in range(80): #total frames captured, captured at rate listed above (supposedly) (20 frames per second? 1/20 * 80 = 4.0s.  May not always be perfectly coordinated w recording time, but should be most of the time
